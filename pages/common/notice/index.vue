		...
		<view class="search-keyword" @touchstart="blur">
			<scroll-view class="keyword-list-box" v-if="isShowKeywordList" scroll-y>
				<view class="keyword-entry" hover-class="keyword-entry-tap" v-for="row in keywordList" :key="row.keyword">
					...
					<view class="keyword-img" @tap="setkeyword(row)"><!--<image src="/static/HM-search/back.png"></image>--></view>
				</view>
			</scroll-view>
			<scroll-view class="keyword-box" v-if="!isShowKeywordList" scroll-y>
				<view class="keyword-block">
					<view class="keyword-list-header">
						<view>搜索历史</view>
						<view><text class="iconfont iconiconfontshanchu1 oldDeleteiconfont" @tap="oldDelete">&#xe69a;</text></view>
					</view>
					<view class="keyword">
						<view v-for="(keyword, index) in oldKeywordList" @tap="doSearch(keyword)" :key="index">{{ keyword }}</view>
					</view>
					<view class="hide-hot-tis" v-if="oldKeywordList.length === 0"><view>暂无记录</view></view>
				</view>
				<view class="keyword-block" v-if="hotKeywordList">
					...
					<view class="keyword" v-if="forbid == true">
						<view v-for="(keyword, index) in hotKeywordList" @tap="doSearch(keyword)" :key="index" v-if="keyword!=''">{{ keyword }}</view>
					</view>
					<view class="hide-hot-tis" v-else><view>当前搜索推荐已隐藏</view></view>
				</view>
			</scroll-view>
		</view>
	</view>
...
.oldDeleteiconfont,.forbidiconfont{
	color: #777;
}
.rf-search {
